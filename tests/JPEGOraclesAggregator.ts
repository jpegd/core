...
describe("JPEGOraclesAggregator", () => {
    ...
    beforeEach(async () => {
        ...
        jpegOraclesAggregator = await JPEGOraclesAggregator.deploy(jpegOracle.address);
    });

    it("should return floor prices from different oracles based on the caller's address", async () => {
        const MockAggregator = await ethers.getContractFactory("MockV3Aggregator");

        await jpegOraclesAggregator.addFloorOracle((await MockAggregator.deploy(18, units(50))).address, owner.address);
        await jpegOraclesAggregator.addFloorOracle((await MockAggregator.deploy(18, units(10))).address, user.address);

        expect(await jpegOraclesAggregator.getFloorETH()).to.equal(units(50));
        expect(await jpegOraclesAggregator.connect(user).getFloorETH()).to.equal(units(10));
    });

    it("should return the correct JPEG price", async() => {
        expect(await jpegOraclesAggregator.callStatic.consultJPEGPriceETH(jpeg.address)).to.equal(1000000000000000);
    });
});
