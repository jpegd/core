...
describe("NFTVault", () => {
	...
	it("should be able to liquidate borrow position without insurance", async () => {
		...
		await expect(nftVault.connect(user).liquidate(4000, owner.address)).to.be.revertedWith(
	...
	it("should be able to liquidate borrow position in strategy with insurance", async () => {
		...
		expect((await nftVault.positions(index)).strategy).to.equal(ZERO_ADDRESS);
