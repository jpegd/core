...
describe("PETHVaultForDAO", () => {
  ...
  beforeEach(async () => {
    ...
    vault = <PETHVaultForDAO>(
      await upgrades.deployProxy(PETHVaultForDAO, [
        peth.address
      ])
    );
  ...
  it("should be able to deposit ETH", async () => {
    ...
    await expect(
      vault.connect(dao).deposit({ value: 0 })
    ).to.revertedWith("invalid_value");
    await vault
      .connect(dao)
  ...
  it("should be able to borrow peth", async () => {
    ...
    expect(await peth.balanceOf(dao.address)).to.equal(
      borrowAmount
    );
