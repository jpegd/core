...
describe("JPEGIndexStaking", () => {
	...
	beforeEach(async () => {
		...
		const JPEGIndexStaking = await ethers.getContractFactory("JPEGIndexStaking");
	...
	it("should allow users to deposit", async () => {
		...
		await jpegIndex.connect(user1).approve(staking.address, user1DepositAmount);
		await jpegIndex.connect(user2).approve(staking.address, user2DepositAmount);

		await staking.connect(user1).deposit(user1DepositAmount);

		expect(await jpegIndex.balanceOf(user1.address)).to.equal(0);
		expect(await jpegIndex.balanceOf(staking.address)).to.equal(user1DepositAmount);
		...
		await staking.connect(user1).deposit(user1DepositAmount);
		expect(await ethers.provider.getBalance(staking.address)).to.equal(units(20));
		expect(await ethers.provider.getBalance(user1.address)).to.be.closeTo(balanceBefore.add(units(20)), 10e16.toString() as any);
	...
	it("should allow users to withdraw", async () => {
		...
		expect(await ethers.provider.getBalance(user2.address)).to.be.closeTo(balanceBefore.add(units(20)), 10e16.toString() as any);
		expect(await staking.pendingReward(user2.address)).to.equal(0);
		expect(await jpegIndex.balanceOf(user2.address)).to.equal(user2DepositAmount);
	...
	it("should allow users to claim rewards", async () => {
		...
		await expect(staking.connect(user1).claim()).to.be.revertedWith("NoRewards()");

		await staking.notifyReward({ value: units(10) });
		const balanceBefore = await ethers.provider.getBalance(user1.address);
		await staking.connect(user1).claim();
		expect(await ethers.provider.getBalance(user1.address)).to.be.closeTo(balanceBefore.add(units(10)), 10e16.toString() as any);
	});
