...
describe("PUSDDAONFTVault", () => {
	let owner: SignerWithAddress,
		dao: SignerWithAddress,
		user: SignerWithAddress;
	let nftVault: PUSDDAONFTVault,
	...
	beforeEach(async () => {
		...
		const NFTVault = await ethers.getContractFactory("PUSDDAONFTVault");
		nftVault = <PUSDDAONFTVault>await upgrades.deployProxy(NFTVault, [
			stablecoin.address,
			erc721.address,
	...
	it("should be able to borrow", async () => {
		await expect(nftVault.connect(user).borrow(10001, 100)).to.be.revertedWith(
			"InvalidNFT(10001)"
		);
	...
	it("should be able to repay", async () => {
		await expect(nftVault.connect(user).repay(10001, 100)).to.be.revertedWith("InvalidNFT(10001)");
		await erc721.mint(user.address, 1);
		await expect(nftVault.connect(user).repay(1, 100)).to.be.revertedWith("Unauthorized()");
	...
	it("should be able to close position", async () => {
		await expect(nftVault.connect(user).closePosition(10001)).to.be.revertedWith(
			"InvalidNFT(10001)"
		);
		await erc721.mint(user.address, 1);
		await expect(nftVault.connect(user).closePosition(1)).to.be.revertedWith("Unauthorized()");
	...
	it("should allow users to execute multiple actions in one call", async () => {
		...
		await nftVault.connect(user).doActions(
			[0, 0, 1, 2],
			[
				abiCoder.encode(["uint256", "uint256"], [index1, borrowAmount1]),
				abiCoder.encode(["uint256", "uint256"], [index2, borrowAmount2]),
	...
	it("organization is deducted from debt", async () => {
		...
		await nftVault.connect(user).borrow(index, units(3000).mul(10));
