...
describe("PETHNFTVault", () => {
    ...
    it("should be able to repay", async () => {
        ...
        await erc721.mint(user.address, 1);
        await expect(nftVault.repay(1, 100)).to.be.revertedWith(
            "Unauthorized()"
        );
    ...
    it("should be able to close position", async () => {
        ...
        await erc721.mint(user.address, 1);
        await expect(nftVault.closePosition(1)).to.be.revertedWith(
            "Unauthorized()"
        );
    ...
    it("should be able to liquidate borrow position without insurance", async () => {
        ...
        await expect(
            nftVault.connect(user).liquidate(index, owner.address)
        ).to.be.revertedWith(
    ...
    it("should allow the router to import positions", async () => {
        ...
        await expect(
            ...
            "InvalidAmount(" + units(2000).mul(1000).toString() + ")"
        );
        await expect(
            nftVault.importPosition(
                user.address,
