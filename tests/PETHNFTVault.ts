...
describe("PETHNFTVault", () => {
  ...
  it("should allow users to lock JPEG to unlock trait boosts", async () => {
    ...
    expect(await nftVault.getNFTValueETH(index)).to.equal(units(50));
    ...
    expect(await nftVault.getNFTValueETH(index)).to.equal(units(500));
  ...
  it("should allow the dao to override floor price", async () => {
    ...
    expect(await nftVault.getNFTValueETH(0)).to.equal(units(10).mul(1));
    await nftVault.connect(dao).disableFloorOverride();
    expect(await nftVault.getNFTValueETH(0)).to.equal(units(50).mul(1));
  });
