...
describe("StrategyPETHConvex", () => {
  ...
  beforeEach(async () => {
    ...
    ethVault = <PETHVaultForDAO>await upgrades.deployProxy(ETHVault, [peth.address]);
    ...
    await ethVault.grantRole(ethers.utils.solidityKeccak256(["string"], ["WHITELISTED_ROLE"]), strategy.address);
  ...
  it("should add liquidity with PETH when harvest is called and curve has less PETH than ETH", async () => {
    ...
    expect(await ethers.provider.getBalance(ethVault.address)).to.equal(units(1.6));
  ...
  it("should add liquidity with ETH when harvest is called and curve has less ETH than PETH", async () => {
    ...
    expect(await ethers.provider.getBalance(want.address)).to.equal(units(1.6));
    expect(await strategy.depositedAssets()).to.equal(units(1002));
  });
});
