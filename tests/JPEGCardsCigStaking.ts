...
describe("JPEGCardsCigStaking", () => {
  ...
  it("should allow users to deposit 1 cig per address", async () => {
    ...
    await expect(cigStaking.deposit(1)).to.be.revertedWith("ERC721: transfer from incorrect owner");
    await cigStaking.deposit(0);
    await cigStaking.connect(user).deposit(1);
    await expect(cigStaking.connect(user).deposit(2)).to.be.revertedWith("CANNOT_STAKE_MULTIPLE");
