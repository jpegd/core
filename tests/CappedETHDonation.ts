...
describe("CappedETHDonation", () => {
	...
	it("should allow the owner to create a new donation event without a whitelist", async () => {
		await expect(donation.newDonationEvent(0, 0, 0, 0, 0, 0)).to.be.revertedWith("InvalidAmount()");
		await expect(donation.newDonationEvent(units(10), 0, 0, 0, 0, 0)).to.be.revertedWith("InvalidAmount()");
		await expect(donation.newDonationEvent(units(10), units(5), 0, 0, 0, 0)).to.be.revertedWith("InvalidStart()");

		const start = (await currentTimestamp()) + 1000;

		await expect(donation.newDonationEvent(units(10), units(5), 0, start, 0, 0)).to.be.revertedWith("InvalidDuration()");
	...
	it("should allow the owner to create a new donation event with a whitelist", async () => {
		...
		await expect(donation.newDonationEvent(units(10), units(5), 0, start, 100, 1000)).to.be.revertedWith("InvalidAmount()");
		
		await donation.newDonationEvent(units(10), units(5), units(1), start, 100, 1000);
	...
	it("should allow whitelisted users to donate", async () => {
		...
		await expect(donation.connect(user1).donateWhitelist({ value: units(1) })).to.be.revertedWith("InactiveDonation()");
		await timeTravel(1000);

		await expect(donation.donateWhitelist({ value: units(1) })).to.be.revertedWith("Unauthorized()");

		await cards.mint(user1.address, 200);

		await expect(donation.connect(user1).donateWhitelist({ value: 0 })).to.be.revertedWith("InvalidAmount()");
		await expect(donation.connect(user1).donateWhitelist({ value: units(2) })).to.be.revertedWith("InvalidAmount()");

		await donation.connect(user1).donateWhitelist({ value: units(.5) });
		...
		await expect(donation.connect(user1).donateWhitelist({ value: units(1) })).to.be.revertedWith("InvalidAmount()");
	...
	it("should allow the owner to end the donation event after reaching cap or end timestamp", async () => {
		...
		await donation.newDonationEvent(units(5), units(5), units(1), start, 0, 1000);
