...
const BAKC_STRATEGY_ROLE = "0x6a1f594db90e5efe37169e9519b7ed208b645cacac4f1018dd1ce6b5cd10bcb1";
...
describe("BAKCApeStakingStrategy", () => {
    ...
    beforeEach(async () => {
        ...
        baycStrategy = <BAYCApeStakingStrategy>await upgrades.deployProxy(BAYCApeStakingStrategy,
            [
                apeStaking.address,
                ape.address,
                bayc.address,
        ...
        const BAKCApeStakingStrategy = await ethers.getContractFactory("BAKCApeStakingStrategy");
        bakcStrategy = <BAKCApeStakingStrategy>await upgrades.deployProxy(BAKCApeStakingStrategy, [], { constructorArgs: [baycStrategy.address, ape.address, apeStaking.address] });
    ...
    it("should allow the vault to deposit NFTs", async () => {
        ...
        await bakcStrategy.afterDeposit(
            user.address,
            nftAmounts.map(e => e.tokenId),
            new AbiCoder().encode(["tuple(uint32 tokenId,uint224 amount)[]"], [nftAmounts])
        );
        ...
        await bakcStrategy.afterDeposit(
            user.address,
            newAmounts.map(e => e.tokenId),
            new AbiCoder().encode(["tuple(uint32 tokenId,uint224 amount)[]"], [newAmounts])
        );
    ...
    it("should allow users to increase the amount of staked tokens after deposit", async () => {
        ...
        const newAmounts = [
            { mainTokenId: nftAmounts[0].tokenId, bakcTokenId: nftAmounts[0].tokenId, amount: units(500) },
            { mainTokenId: nftAmounts[1].tokenId, bakcTokenId: nftAmounts[0].tokenId, amount: units(500) }
        ];
        ...
        await expect(bakcStrategy.depositTokens(newAmounts)).to.be.revertedWith("Unauthorized()");
        await expect(bakcStrategy.connect(user).depositTokens([...newAmounts, { bakcTokenId: 100, mainTokenId: 100, amount: units(500) }])).to.be.reverted;
    ...
    it("should allow users to withdraw tokens", async () => {
        ...
        await expect(bakcStrategy.connect(user).withdrawTokens([{ mainTokenId: 10, bakcTokenId: 10, amount: units(200), isUncommit: false }], user.address)).to.be.reverted;

        const toWithdraw = [
            { mainTokenId: 0, bakcTokenId: 0, amount: units(100), isUncommit: false },
            { mainTokenId: 1, bakcTokenId: 1, amount: units(100), isUncommit: false }
        ];

        await expect(bakcStrategy.withdrawTokens(toWithdraw, user.address)).to.be.revertedWith("Unauthorized()");

        await bakcStrategy.connect(user).withdrawTokens(toWithdraw, user.address);

        const totalWithdrawn = toWithdraw.reduce((s, e) => {
            return e.amount.add(s);
        }, BigNumber.from(0));

        expect(await bakc.ownerOf(toWithdraw[0].bakcTokenId)).to.equal(depositAddress);
        expect(await bakc.ownerOf(toWithdraw[1].bakcTokenId)).to.equal(depositAddress);
    ...
    it("should allow users to claim rewards", async () => {
        ...
        await expect(bakcStrategy.connect(user).claimRewards([{ mainTokenId: 100, bakcTokenId: 100 }], user.address)).to.be.reverted;

        const toClaim = [{ mainTokenId: 0, bakcTokenId: 0 }, { mainTokenId: 1, bakcTokenId: 1 }];

        await expect(bakcStrategy.claimRewards(toClaim, user.address)).to.be.revertedWith("Unauthorized()");
    ...
    it("should allow the vault to withdraw NFTs", async () => {
        ...
        await expect(bakcStrategy.withdraw(owner.address, owner.address, toWithdraw.tokenId)).to.be.revertedWith("Unauthorized()");
        await bakcStrategy.withdraw(user.address, owner.address, toWithdraw.tokenId);
        ...
        await bakcStrategy.connect(user).withdrawTokens([{ mainTokenId: nftAmounts[0].tokenId, bakcTokenId: nftAmounts[0].tokenId, amount: nftAmounts[0].amount, isUncommit: false }], user.address);
        await bakcStrategy.withdraw(user.address, owner.address, nftAmounts[0].tokenId);

        expect(await bakc.ownerOf(nftAmounts[0].tokenId)).to.equal(owner.address);
    ...
    it("should allow legacy deposits to withdraw directly from the strategy", async () => {
        ...
        await bakcStrategy.connect(user).depositTokens([
            { mainTokenId: legacy[0].tokenId, bakcTokenId: legacy[0].tokenId, amount: legacy[0].amount },
            { mainTokenId: legacy[1].tokenId, bakcTokenId: legacy[1].tokenId, amount: legacy[1].amount },
        ]);
                
        await expect(bakcStrategy.withdrawNFTs([legacy[0].tokenId, legacy[1].tokenId], user.address)).to.be.revertedWith("Unauthorized()");
        await expect(bakcStrategy.connect(user).withdrawNFTs([nftAmounts[0].tokenId, nftAmounts[1].tokenId], user.address)).to.be.revertedWith("NotDirectDeposit(" + nftAmounts[0].tokenId + ")");
        await bakcStrategy.connect(user).withdrawNFTs([legacy[0].tokenId, legacy[1].tokenId], user.address);
        ...
        expect(await ape.balanceOf(apeStaking.address)).to.equal(totalAmount.sub(units(300)));
    });
});
