...
describe("JPEGAirdrop", () => {
  ...
  it("should allow setting a past timestamp as the start", async () => {
    ...
    expect(await aJpeg.balanceOf(owner.address)).to.equal(units(1000000000));
    expect(await aJpeg.vestedAmount(owner.address)).to.be.closeTo(units(1000000000).div(2), units(100) as any);

    await aJpeg.release();
    expect(await jpeg.balanceOf(owner.address)).to.be.closeTo(units(1000000000).div(2), units(100) as any);
    expect(await aJpeg.balanceOf(owner.address)).to.be.closeTo(units(1000000000).div(2), units(100) as any);
  });
