...
describe("ApeStakingStrategy", () => {
    ...
    it("should allow users to increase the amount of staked tokens after deposit", async () => {
        ...
        await expect(strategy.depositTokens(newAmounts)).to.be.revertedWith("Unauthorized()");
        await expect(strategy.depositTokens([...newAmounts, { tokenId: 100, amount: units(500) }])).to.be.reverted;
    ...
    it("should allow users to withdraw tokens", async () => {
        ...
        await expect(strategy.connect(user).withdrawTokens([{ tokenId: 10, amount: units(200) }], user.address)).to.be.reverted;
        ...
        await expect(strategy.withdrawTokens(toWithdraw, user.address)).to.be.revertedWith("Unauthorized()");
    ...
    it("should allow users to claim rewards", async () => {
        ...
        await expect(strategy.connect(user).claimRewards([100], user.address)).to.be.reverted;

        const toClaim = [0, 1];

        await expect(strategy.claimRewards(toClaim, user.address)).to.be.revertedWith("Unauthorized()");
    ...
    it("should allow the vault to withdraw NFTs", async () => {
        ...
        expect(await bakc.ownerOf(toWithdraw.bakcTokenId)).to.equal(depositAddress);
        ...
        await strategy.connect(user).withdrawTokens([{ tokenId: nftAmounts[0].mainTokenId, amount: nftAmounts[0].amount }], user.address);
    ...
    it("should allow the BAKC strategy to pair BAKCs with deposited NFTs", async () => {
        ...
        await expect(strategy.connect(user).depositTokensBAKC(user.address, nftAmounts)).to.be.reverted;
    ...
    it("should allow the BAKC strategy to claim rewards from BAKC pairs", async () => {
        ...
        await expect(strategy.connect(user).claimRewardsBAKC(user.address, [{ mainTokenId: nftAmounts[0].mainTokenId, bakcTokenId: nftAmounts[0].bakcTokenId }], user.address)).to.be.reverted;

        await strategy.claimRewardsBAKC(user.address, [{ mainTokenId: nftAmounts[0].mainTokenId, bakcTokenId: nftAmounts[0].bakcTokenId }], user.address)
    ...
    it("should allow the BAKC strategy to withdraw tokens from BAKC pairs", async () => {
        ...
        await expect(strategy.connect(user).withdrawTokensBAKC(user.address, [{ isUncommit: true, ...nftAmounts[0] }])).to.be.reverted;

        await strategy.withdrawTokensBAKC(user.address, [{ isUncommit: true, ...nftAmounts[0] }])

        expect(await ape.balanceOf(apeStaking.address)).to.equal(totalAmount.mul(2).sub(nftAmounts[0].amount));
        expect(await ape.balanceOf(depositAddress)).to.equal(nftAmounts[0].amount);
    });
    ...
    it("should allow the BAKC strategy to transfer BAKC from the clone contract", async () => {
        ...
        await expect(strategy.connect(user).transferBAKC(user.address, [0, 1], user.address)).to.be.reverted;
    ...
    it("should allow the BAKC strategy to transfer apecoin from the clone contract", async () => {
        ...
        await expect(strategy.connect(user).transferApeCoin(user.address, user.address)).to.be.reverted;
